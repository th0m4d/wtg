...
%   Detailed explanation goes here
    ...
    for j=1:size(training_idxs,1)
        path = strcat('data/genres/',folderName ,'/',folderName ,'.',sprintf('%05d',training_idxs(j)), '.au');        
    ...
    for j=1:size(testing_idxs,1)
        path = strcat('data/genres/',folderName ,'/',folderName ,'.',sprintf('%05d',testing_idxs(j)), '.au');
        P = get_cqt_from_audio(path);
